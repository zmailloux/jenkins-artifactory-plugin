...
//        promotionConfig.isDeployBuild()
        ...
        boolean status = PromotionUtils.promoteAndCheckResponse(promotionBuilder.build(), client, listener,
                promotionConfig.getBuildName(), promotionConfig.getBuildNumber(), promotionConfig.isDeployBuild(), promotionConfig.getDeployURL());
        if (!status) {
    ...
    private void logInfo() {
        ...
        if (promotionConfig.isDeployBuild()) {
            strBuilder.append(", including deploy");
        }
